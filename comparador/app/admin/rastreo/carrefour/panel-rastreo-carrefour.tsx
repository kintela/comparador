...
export function PanelRastreoCarrefour() {
  ...
  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm(
        "¿Rastrear Carrefour y guardar sus productos y precios en Supabase?",
      )
    ) {
      return;
    }
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/carrefour", {
        method: "POST",
      ...
      >
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-blue-600 px-4 py-3 font-bold text-white hover:bg-blue-500 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar Carrefour"}
          </button>
          <button
            type="button"
            disabled={cargando}
          ...
          >
            Rastrear y guardar Carrefour
          </button>
        </div>
        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: buscador oficial Carrefour para 48980, punto de venta 005457.
          La consulta se realiza sin reutilizar cookies del navegador.
        </p>
      </form>
