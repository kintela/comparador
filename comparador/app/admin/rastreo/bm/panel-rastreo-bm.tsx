...
export function PanelRastreoBm() {
  ...
  const [resultado, setResultado] = useState<ResultadoBm | null>(null);

  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm("¿Rastrear BM Online y guardar productos y precios en Supabase?")
    ) {
      return;
    }
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/bm", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          consultas: consultasTexto.split("\n").map((item) => item.trim()).filter(Boolean),
          resultadosPorConsulta,
          maxProductos,
          guardar,
        }),
      });
      setResultado((await respuesta.json()) as ResultadoBm);
    } catch {
      ...
      >
        <label className="block">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-yellow-400"
          />
        </label>
        ...
        <label className="mt-5 block">
          ...
          <textarea
            ...
            className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 font-mono text-sm outline-none focus:border-yellow-400"
          />
        </label>
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-yellow-400 px-4 py-3 font-bold text-slate-950 hover:bg-yellow-300 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar BM"}
          </button>
          <button
            type="button"
            disabled={cargando}
            onClick={() => void ejecutar(true)}
            className="rounded-lg border border-yellow-500/60 px-4 py-3 font-bold text-yellow-300 hover:bg-yellow-500/10 disabled:opacity-50"
          >
            Rastrear y guardar BM
          </button>
        </div>

        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: catálogo BM Online general. No se atribuye a un centro físico.
        </p>
      </form>
