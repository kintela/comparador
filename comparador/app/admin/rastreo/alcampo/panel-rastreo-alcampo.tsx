...
export function PanelRastreoAlcampo() {
  ...
  const [resultado, setResultado] = useState<ResultadoAlcampo | null>(null);

  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm(
        "¿Rastrear Alcampo Online y guardar sus productos y precios en Supabase?",
      )
    ) return;
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/alcampo", {
        method: "POST",
        headers: {
      ...
      setResultado((await respuesta.json()) as ResultadoAlcampo);
    } catch {
      ...
      >
        <label className="block">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-orange-400"
          />
        </label>
        ...
        <label className="mt-5 block">
          ...
          <textarea
            ...
            className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 font-mono text-sm outline-none focus:border-orange-400"
          />
        </label>
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-orange-500 px-4 py-3 font-bold text-slate-950 hover:bg-orange-400 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar Alcampo"}
          </button>
          <button
            type="button"
            disabled={cargando}
            onClick={() => void ejecutar(true)}
            className="rounded-lg border border-orange-500/60 px-4 py-3 font-bold text-orange-300 hover:bg-orange-500/10 disabled:opacity-50"
          >
            Rastrear y guardar Alcampo
          </button>
        </div>

        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: catálogo oficial Alcampo Online. La región interna aparecerá
          tras previsualizar; no equivale necesariamente a una tienda concreta.
        </p>
      </form>
        ...
        {resultado?.ok && (
          <>
            ...
            <div className="mb-5 rounded-xl border border-slate-700 bg-slate-950 p-4 text-sm text-slate-300">
              Origen de precios: {resultado.origenPrecios ?? "Alcampo Online"}
              {resultado.regionId && (
                <span className="mt-1 block text-xs text-slate-500">
                  Región interna: {resultado.regionId}
                </span>
              )}
            </div>
