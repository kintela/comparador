...
export function PanelRastreoDia() {
  ...
  const [resultado, setResultado] = useState<ResultadoDia | null>(null);

  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm(
        "¿Rastrear DIA Online y guardar sus productos y precios en Supabase?",
      )
    ) {
      return;
    }
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/dia", {
        method: "POST",
        headers: {
      ...
      setResultado((await respuesta.json()) as ResultadoDia);
    } catch {
      ...
      >
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-red-500 px-4 py-3 font-bold text-white hover:bg-red-400 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar DIA"}
          </button>
          <button
            type="button"
            disabled={cargando}
          ...
          >
            Rastrear y guardar DIA
          </button>
        </div>

        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: catálogo oficial DIA Online. La zona de precios se mostrará
          después de previsualizar y no representa necesariamente una tienda
          física concreta.
        </p>
      </form>
        ...
        {resultado?.ok && (
          <>
            ...
            <div className="mb-5 rounded-xl border border-slate-700 bg-slate-950 p-4 text-sm text-slate-300">
              Origen de precios: {resultado.origenPrecios ?? "DIA Online"}
            </div>
