...
export function PanelRastreoLidl() {
  ...
  const [resultado, setResultado] = useState<ResultadoLidl | null>(null);

  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm(
        "¿Rastrear Lidl para la región Vizcaya y guardar productos y precios en Supabase?",
      )
    ) {
      return;
    }
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/lidl", {
        method: "POST",
        headers: {
      ...
      setResultado((await respuesta.json()) as ResultadoLidl);
    } catch {
      ...
      >
        <label className="block">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-blue-400"
          />
        </label>
        ...
        <label className="mt-5 block">
          ...
          <textarea
            ...
            className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 font-mono text-sm outline-none focus:border-blue-400"
          />
        </label>
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-blue-500 px-4 py-3 font-bold text-white hover:bg-blue-400 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar Lidl"}
          </button>
          <button
            type="button"
            disabled={cargando}
            onClick={() => void ejecutar(true)}
            className="rounded-lg border border-blue-500/60 px-4 py-3 font-bold text-blue-300 hover:bg-blue-500/10 disabled:opacity-50"
          >
            Rastrear y guardar Lidl
          </button>
        </div>

        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: buscador oficial de Lidl España, filtrado por alimentación y
          región Vizcaya. Puede contener principalmente productos publicados y
          ofertas semanales.
        </p>
      </form>
        ...
        {resultado?.ok && (
          <>
            ...
            <div className="mb-5 rounded-xl border border-slate-700 bg-slate-950 p-4 text-sm text-slate-300">
              Origen de precios: {resultado.origenPrecios ?? "Lidl Web · Vizcaya"}
            </div>
