...
export function PanelRastreoPrimaprix() {
  ...
  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm(
        "¿Rastrear el catálogo web de Primaprix y guardar sus productos y precios en Supabase?",
      )
    ) {
      return;
    }
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/primaprix", {
        method: "POST",
      ...
      >
        <label className="block">
          ...
          <textarea
            ...
            className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 font-mono text-sm outline-none focus:border-pink-400"
          />
        </label>
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-pink-600 px-4 py-3 font-bold text-white hover:bg-pink-500 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar Primaprix"}
          </button>
          <button
            type="button"
            disabled={cargando}
            onClick={() => void ejecutar(true)}
            className="rounded-lg border border-pink-500/60 px-4 py-3 font-bold text-pink-300 hover:bg-pink-500/10 disabled:opacity-50"
          >
            Rastrear y guardar Primaprix
          </button>
        </div>
        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: los cinco perfiles del catálogo oficial de Primaprix. Los
          precios son una muestra disponible en tiendas y no confirman stock
          local en Santurtzi.
        </p>
      </form>
                  ...
                  {productos.map((producto) => (
                    <tr key={producto.identificadorExterno}>
                      ...
                      <td className="px-4 py-3">
                        {producto.disponible ? "Publicado" : "No disponible"}
                      </td>
                      <td className="px-4 py-3 text-right font-mono font-bold text-pink-300">
