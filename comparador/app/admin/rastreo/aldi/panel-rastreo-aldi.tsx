...
export function PanelRastreoAldi() {
  ...
  const [resultado, setResultado] = useState<ResultadoAldi | null>(null);

  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm(
        "¿Rastrear ALDI Web Península y guardar productos y precios en Supabase?",
      )
    ) {
      return;
    }
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/aldi", {
        method: "POST",
        headers: {
      ...
      setResultado((await respuesta.json()) as ResultadoAldi);
    } catch {
      ...
      >
        ...
        <label className="mt-5 block">
          ...
          <textarea
            ...
            className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 font-mono text-sm outline-none focus:border-cyan-400"
          />
        </label>
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-cyan-400 px-4 py-3 font-bold text-slate-950 hover:bg-cyan-300 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar ALDI"}
          </button>
          <button
            type="button"
            disabled={cargando}
            onClick={() => void ejecutar(true)}
            className="rounded-lg border border-cyan-500/60 px-4 py-3 font-bold text-cyan-300 hover:bg-cyan-500/10 disabled:opacity-50"
          >
            Rastrear y guardar ALDI
          </button>
        </div>

        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: catálogo web regional de Península. Tienda de referencia:
          ALDI Santurtzi, Antonio Alzaga Hiribidea 73b.
        </p>
      </form>
