...
export function PanelRastreoElCorteIngles() {
  ...
  async function ejecutar(guardar: boolean) {
    if (
      guardar &&
      !window.confirm(
        "¿Rastrear Supermercado El Corte Inglés y guardar sus productos y precios en Supabase?",
      )
    ) {
      return;
    }
    ...
    setResultado(null);
    try {
      const respuesta = await fetch("/api/rastreo/el-corte-ingles", {
        method: "POST",
      ...
      >
        ...
        <div className="mt-6 grid gap-3">
          <button
            type="submit"
            disabled={cargando}
            className="rounded-lg bg-emerald-500 px-4 py-3 font-bold text-slate-950 hover:bg-emerald-400 disabled:opacity-50"
          >
            {cargando ? "Rastreando…" : "Previsualizar El Corte Inglés"}
          </button>
          <button
            type="button"
            disabled={cargando}
          ...
          >
            Rastrear y guardar El Corte Inglés
          </button>
        </div>
        <p className="mt-4 text-xs leading-5 text-slate-500">
          Origen: catálogo oficial de Supermercado El Corte Inglés. La
          previsualización indicará el centro de entrega online de referencia.
        </p>
      </form>
        ...
        {resultado?.ok && (
          <>
            ...
            <div className="mb-5 rounded-xl border border-slate-700 bg-slate-950 p-4 text-sm text-slate-300">
              Origen de precios: {resultado.origenPrecios ?? "El Corte Inglés Online"}
            </div>
