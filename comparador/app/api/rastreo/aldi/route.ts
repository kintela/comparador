...
export async function POST(request: Request) {
  ...
  try {
    const resultado = await rastrearLoteAldi({
      consultas,
      resultadosPorConsulta,
      maxProductos,
    });
    const persistencia = guardar
      ? await guardarRastreoAldi({
    ...
    return Response.json({
      ok: true,
      guardado: guardar,
      origenPrecios: "ALDI Web · Península",
      tiendaReferencia: {
        id: "3604787",
        codigo: "ES007083",
        nombre: "ALDI Santurtzi",
        direccion: "Antonio Alzaga Hiribidea 73b, 48980 Santurtzi",
      },
