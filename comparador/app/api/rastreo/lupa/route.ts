...
export async function POST(request: Request) {
  ...
  let cuerpo: SolicitudRastreoLupa;
  return ejecutarConBloqueoRastreo("lupa", async () => {
    try {
    cuerpo = (await request.json()) as SolicitudRastreoLupa;
  } catch {
  ...
  try {
    const resultado = await rastrearLoteLupa({
      consultas,
      resultadosPorConsulta,
      maxProductos,
    });
    const persistencia = guardar
      ? await guardarRastreoLupa({
    ...
    return Response.json({
      ok: true,
      guardado: guardar,
      origenPrecios: "Lupa Online · Santander",
    ...
    const mensaje =
      error instanceof Error
        ? error.message
        : "Error desconocido durante el rastreo Lupa";
    console.error("Error en el rastreo manual de Lupa:", mensaje);
