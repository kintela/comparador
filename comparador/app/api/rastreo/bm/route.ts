...
export async function POST(request: Request) {
  ...
  if (
    ...
    return Response.json(
      {
        ok: false,
        error: "Los resultados por búsqueda deben estar entre 1 y 20 y el máximo entre 1 y 300",
      },
      { status: 400 },
    );
  }
  ...
  try {
    const resultado = await rastrearLoteBm({
      consultas,
      resultadosPorConsulta,
      maxProductos,
    });
    const persistencia = guardar
      ? await guardarRastreoBm({
    ...
    return Response.json({
      ok: true,
      guardado: guardar,
      origenPrecios: "BM Online general",
