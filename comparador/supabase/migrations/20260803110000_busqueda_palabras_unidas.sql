...
as $$
  ...
  where referencia.activo = true
    ...
    and (
      extensions.unaccent(lower(referencia.nombre_original)) like
        '%' || extensions.unaccent(lower(trim(p_consulta))) || '%'
      or regexp_replace(
        extensions.unaccent(lower(referencia.nombre_original)),
        '[^[:alnum:]]+',
        '',
        'g'
      ) like '%' || regexp_replace(
...
comment on function public.buscar_ids_productos_supermercado(
  ...
  'Localiza referencias ignorando tildes, mayusculas y separadores internos de palabras.';
