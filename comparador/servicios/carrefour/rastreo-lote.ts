...
import { rastrearProductosCarrefour } from "./cliente-carrefour";
import type {
  ErrorRastreoCarrefour,
  ProductoCarrefour,
} from "./tipos-carrefour";

const PAUSA_ENTRE_BUSQUEDAS_MS = 250;

function esperar(milisegundos: number) {
  return new Promise((resolve) => setTimeout(resolve, milisegundos));
}

export async function rastrearLoteCarrefour({
  consultas,
  ...
  const errores: ErrorRastreoCarrefour[] = [];
  ...
  for (const consulta of consultas) {
    ...
    try {
      const resultado = await rastrearProductosCarrefour({
        consulta,
        limite: Math.min(
          resultadosPorConsulta,
  ...
  if (productos.size === 0 && !permitirVacio) {
    throw new Error("El rastreo de Carrefour no devolvió ningún producto válido");
  }
