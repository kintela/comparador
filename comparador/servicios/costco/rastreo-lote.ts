...
export async function rastrearLoteCostco({
  consultas,
  ...
  for (const consulta of consultas) {
    ...
    try {
      const resultado = await rastrearProductosCostco({
        consulta,
        limite: Math.min(
          resultadosPorConsulta,
  ...
  if (productos.size === 0 && !permitirVacio) {
    throw new Error("El rastreo de Costco no devolvió ningún producto válido");
  }
  return {
